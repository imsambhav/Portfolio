...
$(document).ready(function() {
	...
	========================================================================== */
	if (
		/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) === false &&
		detectIE() === false
	) {
		...
		$flickr_images.jflickrfeed(
			{
				...
				itemTemplate:
					'<li class="flickr-image"><a href="{{image_b}}"><img src="{{image_b}}" alt="{{title}}" /></a></li>'
			},
			function() {
	...
	var globalOwlOptions = {
		navRewind: false,
		navClass: [
			"owl-prev mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--fab",
			"owl-next mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--fab"
		],
		navText: [
	...
	$(".skill-slider")
		...
		.wrapAll("<div class='owl-stage-outer-wrapper' />");

	/* HAMBURGER
    ========================================================================== */
	$(".mobile-menu-trigger").on("click", function(e) {
		$(this)
			.toggleClass("mobile-menu-trigger--clicked")
			.find(".hamburger")
	...
	$(document).scroll(function() {
		$(".mobile-menu-trigger").removeClass("is-active mobile-menu-trigger--clicked");
	...
	$(".single-skill").waypoint({
		handler: function(event, direction) {
			$(this)
				.find(".progress-bar__bar__active")
				.each(function() {
					var width = $(this)
						.closest(".progress-bar")
						.find(".progress-bar__label span")
						.html();
					$(this).width(width);
				});
		},
		...
		if (sections.length > 0) {
			sections.waypoint({
				handler: function(event, direction) {
					...
					var active_link = $(
						'.menu-smooth-scroll[href="#' + active_section.attr("id") + '"]'
					);
					active_link
						.parent("li")
						.addClass("active")
	...
	// Changing Contact Address Fields
	$("#single-address__text_email").text("dang.dennis9@gmail.com");
	$("#single-address__text_phone")
		.text("510-206-7785")
		.parent()
		.attr("href", "tel:+1-510-206-7785");
	...
	var mapWrapperID = "#map",
		draggableOp = $.browser.mobile === true ? false : true;

	if ($(mapWrapperID).length > 0 && window.google && window.google.maps) {
		window.mapOps = {
			lat: 33.7175, // Provide your latitude
			lng: -117.8311, // Provide your longitude
			content: '<p class="mapLocation">Orange County, CA, USA</p>', // Provide your address to show on pop up
		...
		portfolioArea.each(function() {
			...
			var loadData = function(ajaxItems, $btn) {
				if (ajaxItems) {
					var lists,
						loadedIds = [],
						target = $this.find(".portfolio-category .filter.active").data("target");
